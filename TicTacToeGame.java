...
public class TicTacToeGame extends JPanel {
	...
	int selY = 0;
    /* 
    Choose four colours for GUI
    We can find hex values for others as well.
    */

	Color seaGreen = new Color(0x80bdab);
	Color sandyBrown = new Color(0xfdcb9e);
	Color offWhite = new Color(0xf7f7f7);
	Color charcoalGray = new Color(0x3f3f44);
	...
	public void GamePanel() {
		Dimension size = new Dimension(415, 295);
		setPreferredSize(size);
		setMaximumSize(size);
		setMinimumSize(size);
		addMouseListener(new XOListener());
		jButton = new JButton("Do You Want To Play Again?");
    ...
    public void resetGame() {
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				board[i][j] = 0; // It shows that all spots on the board are empty or set to zero first
			}
		}
    ...
    public void drawUI(Graphics page) {
		...
		// DRAW THE score X
		ImageIcon xIcon = new ImageIcon("greenx.png");
		...
		page.setColor(charcoalGray);
		page.fillOval(49 + 190 + offset, 85, 19, 19);

		// DRAW WHOSE TURN or WINNER
		page.setColor(offWhite);
    ...
    public static void main(String[] args) {
        JFrame jfr = new JFrame("Tic Tac Toe Game");
        jfr.getContentPane();
